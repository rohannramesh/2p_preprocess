...
% edited RR for sbx files 140727
...
elseif strcmpi(ext,'.sbx')
    [mouse,date,runs] = get_mouse_date_runs_from_path(path_to_file);
    dirs = sbxDirs(mouse,date,runs);
    load([dirs.runs{1}.path '\' dirs.runs{1}.sbx_name '.dparams'],'-mat');
    imData = sbxLoadReg(dirs.runs{1},params,sframe,num2read);
    % make edges 0
    edges = [70 70 25 25];
    imData = imData(edges(3):end-edges(4)-1,edges(1):end-edges(2)-1,:);
%     imData(1:edges(3),:,:) = 0;
%     imData(end-edges(4)+1:end,:,:) = 0;
%     imData(:,1:edges(1),:) = 0;
%     imData(:,end-edges(2)+1:end,:) = 0;    
else
