function [icaguidata] = rederive_icaguidata_with_selective_criterion(MultiRunDirs,data,A,C,keep,ROIvars)
%%
...
tc_tmp =  nan(size(ROIvars.tc,2),size(A,2)); % for cc in the future
...
for ii = 1:length(sorted_ind_keep)
    ...
    tc_tmp(:,indrr) = detrend(ROIvars.tc(sorted_ind_keep(ii),:));
...
for curr_ROI_tmp = 1:indrr-1
    ...
    if sum(icaguidata.tc_cc(curr_ROI_tmp,curr_ROI_tmp+1:end) > 0.65) > 0
