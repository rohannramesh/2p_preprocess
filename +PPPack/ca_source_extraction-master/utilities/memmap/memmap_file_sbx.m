function data = memmap_file_sbx(filename,sframe,num2read,chunksize)
...
if isempty(chunksize)
    ...
    % get dirs from filename
    tmp{1} = filename;
    [mouse,date,runs] = get_mouse_date_runs_from_path(filename);
    dirs = sbxDirs(mouse,date,runs);
    info = sbxInfo(filename, 1);
...
%     Yt = imread(filename,'Index',1,'Info',info);
    Yt = sbxReadPMT(filename,1,1);
    edges = [70 70 25 25];
    Yt = Yt(edges(3):end-edges(4)-1,edges(1):end-edges(2)-1,:);
