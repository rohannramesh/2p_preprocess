function last_pc_above_noise = CellsortPlotPCspectrum_RR(mov, CovEvals, PCuse, batch)
...
if ~batch
	...
	plot(threshold*noiseigs / noiseigs(1), 'b--', 'LineWidth',2)
	if ~isempty(PCuse)
...
%     try
%         j=j+jstep;
%         imread(fn,j); % Problem RR - not reading in correctly
%     catch
%         if jstep>1
%             j=j-jstep;
%             jstep = jstep/10;
