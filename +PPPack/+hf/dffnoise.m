...
% returns
    % noise,  mu, sigma, fitdata = getdffnoise(x, y)
    % noise, mu, sigma, k, scale, xoffset = getdffnoise(x, y)
    % noise, mu, sigma, k, scale, xoffset, fitdata = getdffnoise(x, y)
...
% chi-square PDF. 
    ...
    [xd, yd] = PPPack.hf.dffhistogram(dff);
    ...
    if nargout > 3
        fun = @(x, xdata)chinorm(xd, mu_, sigma_, x(1), x(2), x(3), noise_);
