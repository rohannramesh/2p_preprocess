...
%   NEWTYPE - string. cast stack to newType if not emptyedd
%
...
% 16/11/12 VB filter out non tifs before applying indices as filter
...
for iF = 1:nFiles
	...
	%Change by MJLM: this used to be fullfileMH, which uses the wrong
	%filesep for some silly reason, which meant that imageJ didn't
	%understand paths that were passed to it.
...
totS = toc-totalT;
fprintf('\nloaded %i frames (%ix%i pixels) in %2.1f seconds (%2.0f fps or %5.0f MB/s )\n', ...
        iframe, fi.height, fi.width, totS, iframe/totS,s.bytes/1024^2/totS);
